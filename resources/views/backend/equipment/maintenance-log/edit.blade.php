    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                ...
                @endif
                <form id="maintenance-log-form" action="{{ route('maintenance-log.update', ['equipment_qr_code_id' => $equipment_qr_code->id, 'id' => $log->id]) }}" method="POST" novalidate>
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
                        ...
                        <select class="form-select choices" id="maintenance_type_id" name="maintenance_type_id">
                            ...
                            @foreach($maintenanceTypes as $type)
                                <option value="{{ $type->id }}" {{ $log->maintenance_type_id == $type->id ? 'selected' : '' }}>{{ $type->name ?? $type->title }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control flatpickr" id="start_date" name="start_date" value="{{ $log->start_date->format('Y-m-d') }}">
                        @error('start_date') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="end_date" class="form-label">Ngày kết thúc bảo trì <span class="text-danger">*</span></label>
                        <input type="text" class="form-control flatpickr" id="end_date" name="end_date" value="{{ $log->end_date ? $log->end_date->format('Y-m-d') : '' }}">
                        @error('end_date') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="performer" class="form-label">Người thực hiện <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="performer" name="performer" value="{{ $log->performer }}">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="description-input" class="d-none" name="description">{{ $log->description }}</textarea>
                        @error('description') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="status" class="form-label">Trạng thái <span class="text-danger">*</span></label>
                        <select class="form-select choices" id="status" name="status">
                            <option value="operating_active" {{ $log->status == 'operating_active' ? 'selected' : '' }}>Đang hoạt động</option>
                            <option value="under_repair" {{ $log->status == 'under_repair' ? 'selected' : '' }}>Đang sửa chữa</option>
                            <option value="broken_damaged" {{ $log->status == 'broken_damaged' ? 'selected' : '' }}>Đã hỏng</option>
                        </select>
                        @error('status') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <div class="mb-3">
                        <label for="setup_time" class="form-label">Cài đặt thời gian</label>
                        <input type="text" class="form-control flatpickr" id="setup_time" name="setup_time" value="{{ $log->setup_time ? $log->setup_time->format('Y-m-d') : '' }}">
                        @error('setup_time') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    <button type="submit" class="btn btn-primary">Cập nhật nhật ký bảo dưỡng</button>
                </form>
            </div>
        </div>
    </div>
