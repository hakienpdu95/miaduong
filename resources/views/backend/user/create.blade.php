        ...
        <div class="card">
            <div class="card-body">
                ...
                <form id="user-form" action="{{ route('user.store') }}" method="POST" novalidate>
                    ...
                    <div class="mb-3">
                        <label for="unit_id" class="form-label">Đơn vị <span id="unit-required-star" class="text-danger" style="display: none;">*</span></label>
    ...
    <script>
        window.addEventListener('load', function() {
            $(document).ready(function() {
                // Toggle name field and unit required star based on account_type
                $('#account_type').on('change', function() {
                    const value = $(this).val() || '';
                    if (value === 'warehouse_management') {
                        $('#name-field').show();
                        $('#unit-required-star').hide(); // Ẩn * cho unit_id
                    } else {
                        $('#name-field').hide();
                        $('#name').val(''); // Clear value if hidden
                        $('#unit-required-star').show(); // Hiện * cho unit_id
                    }
                });
                ...
                if (initialAccountType !== 'warehouse_management') {
                    $('#name-field').hide();
                    $('#unit-required-star').show();
                } else {
                    $('#unit-required-star').hide();
                }

                $('#user-form').on('submit', function(e) {
                    let valid = true;
                    const requiredFields = ['account_type', 'username', 'password', 'password_confirmation'];
                    const errors = [];
                    ...
                    // Conditional check for unit_id (only required if account_type is foreman)
                    const accountTypeVal = $('#account_type').val() || '';
                    const unitInput = $('#unit_id');
                    const unitValue = unitInput.val() || '';
                    const unitLabel = $('label[for="unit_id"]').text().replace(/\s*\*\s*$/, '').trim();
                    if (accountTypeVal === 'foreman' && !unitValue.trim()) {
                        valid = false;
                        errors.push(`${unitLabel} là bắt buộc!`);
                        unitInput.closest('.choices')?.addClass('is-invalid') || unitInput.addClass('is-invalid');
                    } else {
                        unitInput.closest('.choices')?.removeClass('is-invalid') || unitInput.removeClass('is-invalid');
                    }
